...
import styles from "./PublishedProducts.module.scss";
...
import reStock from "../../../assets/icons/product/re-instock.svg";
...
import soldOutIcon from "../../../assets/icons/product/soldout.svg";
...
function PublishedProducts() {
  ...
  }, [sortState, pageIndex, pageSize, isReload]);
  ...
  const handleRestockButton = (product) => {
    setSelectedProduct(product);
    setShowRestock((prev) => !prev);
  };
  const handleSoldOutButton = async (productId) => {
    try {
      await axios.patch(
        `http://localhost:3002/product/${productId}/sold-out`,
        {},
        {
          withCredentials: true,
        }
  ...
  const handleHideButton = async (productId) => {
    try {
      await axios.patch(
        `http://localhost:3002/product/${productId}/hidden`,
        {},
        {
          withCredentials: true,
        }
                    ...
                    <>
                      ...
                      <td>
                        <div className={cx("action-buttons")}>
                          <button onClick={() => handleRestockButton(product)}>
                            <img src={reStock} alt="re-Stock Icon" />
                            <span>Restock</span>
                          </button>
                          <button
                            onClick={() => handleSoldOutButton(product._id)}
                          >
                            <img src={soldOutIcon} alt="Sold Out Icon" />
                            <span>Sold Out</span>
                          </button>
                          <button onClick={() => handleHideButton(product._id)}>
                            <img src={hideIcon} alt="Hide Product Icon" />
                            <span>Hide</span>
                          </button>
                        </div>
                      </td>
